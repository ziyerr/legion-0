...
# 或
cc l1 磐石                          # 快捷命令：启动/恢复 Claude L1
cx l1 玄武                          # 快捷命令：启动/恢复 Codex L1
```
...
| `cc l1 [名]` | 启动/恢复 Claude L1；创建新 L1 时执行 Claude 军团初始化并接入通讯，不展开项目模板 |
| `cx l1 [名]` | 启动/恢复 Codex L1；创建新 L1 时执行 Codex 军团初始化并接入通讯，不展开项目模板 |
...
| `claudel1 <名>` / `codexl1 <名>` | 兼容入口；新文档统一使用 `cc l1` / `cx l1` |
...
cc l1 青龙军团 --no-attach
cx l1 玄武军团 --no-attach
cc l1+1 青龙军团 --no-attach
cx l1+1 玄武军团 --no-attach

# 如果 PATH 尚未包含 Legion scripts，先执行：
...
# 兼容入口
~/.claude/scripts/cc l1 青龙军团
~/.claude/scripts/cx l1 玄武军团
~/.claude/scripts/claude l1 青龙军团
~/.claude/scripts/codex l1 玄武军团

# cc/cx/claude/codex shim 只拦截 l1 / l1+1；cc 的其他参数会转发给真实 C 编译器，codex/claude 的其他参数会转发给真实 CLI
...
`legion 0` 是唯一的全局/项目初始化入口：它负责展开项目模板、memory、skills、tools，注册项目目录，并预建 mixed 持久化通讯链。`cc l1` / `cx l1` 只负责启动或恢复对应 provider 的 L1；如果创建新 L1，会执行 provider 专属军团初始化并接入 mixed 通讯，不复制项目模板。旧的 `claudel1` / `codexl1`、`claude l1` / `codex l1` 仍保留兼容。

Codex L1 启动后会收到统一控制面提示：创建下属 worker 时通过 `legion.sh mixed campaign`，并在计划里用 `"provider": "claude"` 或 `"provider": "codex"` 明确调度哪类军团成员。Claude L1 通过 `legion.sh claude h` 或 `cc l1 <名>` 启动时也会登记到同一个 mixed registry，因此同项目里的 Claude L1 和 Codex L1 可以互相看见，围绕同一份任务态势协同。默认启动不会把两个 L1 塞进同一个 `legion-view` 分屏；它们是两个独立 tmux session，需要合并观察时才显式运行 `legion view`。
...
# view 布局：左侧 L1 保持可交互；右侧所有 L2 等高堆叠，每个 L2 有独立伪随机彩色边框和底部标题
...
动态扩编出来的 L2 默认是 campaign 生命周期：名下任务全部成功完成且没有要求保留上下文时，Legion Core 会发送 `DISBAND:init-complete`，关闭对应 tmux session，并把 L2 标记为 `completed`。如果某个 L1 退出或被标记为终态，Legion Core 会只沿该 L1 的 parent 树解散下级 L2、阻断其未终态任务并关闭可达的 L2 tmux session，不影响 peer L1 的团队。`legion view` 默认只显示双 L1 和仍持有未结束任务的动态 L2；已完成但因上下文保留而留存的 L2 不会挤占默认作战面。如果该 L2 的上下文还要继续保留，在任务里声明：
...
L2 指挥官同样在 tmux 中可见，并写入 `mixed-registry.json` 和 `events.jsonl`。这保证 `cc l1 青龙` 与 `cx l1 玄武` 同时存在时，双方看到的是同一套战场态势。
