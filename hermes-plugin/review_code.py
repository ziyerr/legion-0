...
def run(args: Dict[str, Any], **kwargs) -> str:
    ...
    # 没有 project_id 也写一行，避免 KR 度量丢失（W-3 修复 P1.6）：
    # 旧实现统一用字面值 "no-project-id" → 跨 PR 写入 list_reviews 同一域，污染 KR 分组。
    # 新实现：把 pr_url md5 截前 8 位作为伪 project_id（每个 PR 唯一），
    # daily_brief 仍可按 list_reviews(orphan-pr-xxx) 拿回单 PR 的 review 历史。
    if project_id:
        review_project_id = project_id
    else:
        pr_hash = hashlib.md5(pr_url.encode("utf-8")).hexdigest()[:8]
        review_project_id = f"orphan-pr-{pr_hash}"

    # W-2 修复 P1.6：把 create_review 包进 retry_with_backoff，
    # 抗 sqlite "database is locked"（多 profile 并发写共享 dev.db），
    # base_delay=0.2 比 git init 短，sqlite 锁通常很快释放。
    def _do_create_review() -> Dict[str, Any]:
        try:
            return adr_storage.create_review(
            ...
            level = error_classifier.classify(e)
            # sqlite locked 等技术级 → 重试；其它（permission/intent/unknown）立即抛
            raise _ReviewCodeError(f"create_review failed: {e}", level=level)

    try:
        review_row = error_classifier.retry_with_backoff(
            _do_create_review, max_retries=3, base_delay=0.2
        )
...
def _normalize_checklist(raw: List[Any]) -> List[Dict[str, Any]]:
    ...
    for item_no, name in CHECKLIST_ITEMS:
        ...
        status_raw = str(entry.get("status") or "").strip().upper()
        # G-1 修复 P1.6：先做空格归一化（'NON BLOCKING' → 'NON-BLOCKING'）
        status = re.sub(r"\s+", "-", status_raw)
        if status not in ALLOWED_STATUS:
            # 非法 status 兜底为 NON-BLOCKING（保守 — 让人看到"评论"而非"通过"）
            # 旧实现兜底成 PASS，会把 LLM 不确定的判断变成"通过"，伪幻觉；
            # 改成 NON-BLOCKING 让人审，保留信号
            status = "NON-BLOCKING"
...
def _enforce_blocking_format(
    ...
    G-4 修复 P1.6：旧实现 reformat 兜底文案 "建议把当前实现改成 ... 因为 ..."
    虽然关键词命中，但**没有真实 X→Y 内容**——文案校验自循环（reformat 后
    `_has_blocking_format` 仍 True，但其实并未给出可执行修复指令）。
    新实现：reformat 后再次 `_has_blocking_format` 检测，若仍命中关键词但**原文
    确实没有可执行修复指令**（即原 comment 未含格式关键词）→ 强制把 status 降级
    为 NON-BLOCKING + 加更醒目 warning。理由：BLOCKING 是硬 gate，应仅在
    程小远给出真实 X→Y 时使用。
    """
    out: List[Dict[str, Any]] = []
    warnings: List[str] = []
    for item in checklist:
        if item["status"] == "BLOCKING" and not _has_blocking_format(item["comment"]):
            ...
            # G-4 修复：BLOCKING 应仅在有真实 X→Y 时使用；reformat 兜底文案虽然
            # 关键词命中（自循环），实际不能指导修复 → 强制降级为 NON-BLOCKING
            new_item["status"] = "NON-BLOCKING"
            new_item["downgraded_due_to_unformatted_blocking"] = True
            out.append(new_item)
            warnings.append(
                f"item {item['item']} ({item['name']}) BLOCKING 文案不合规且无真实 X→Y "
                f"内容 → 已降级为 NON-BLOCKING（原文：{original[:80]}）"
            )
        else:
            out.append(item)
    return out, warnings
    ...
    if new_status == "maintained":
        try:
            escalation_result = error_classifier.escalate_to_owner(
                # W-1 修复 P1.6：appeal 维持 = 决策权冲突，按未知保守升级（R-NFR-22），
                # 不是技术鉴权失败 → 不该归 LEVEL_PERMISSION（避免污染 KR 度量分母）
                error_classifier.LEVEL_UNKNOWN,
...
def _fetch_code_review(code_review_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        # fix W-2 reviewer-p1-7：用 mode=ro 只读 URI，与同模块其他读路径一致
...
def find_stale_blocking_reviews(stale_hours: float = 24.0) -> List[Dict[str, Any]]:
    ...
    G-5 修复 P1.6：旧实现用 SQL 字符串字典序比较（`"reviewed_at" < ?`），
    依赖 `_now_iso()` 写入的 ISO 8601 字面格式（`Z` 后缀 vs `+00:00`、有无毫秒）
    完全一致才正确。改为：先用 SQL 拉所有候选行（仅 blocker_count > 0 + appeal=none
    过滤），在 Python 里用 `datetime.fromisoformat` 解析后做真实时间比较。
    """
    import sqlite3 as _sqlite
    ...
    for r in rows:
        d = dict(r)
        ts_raw = d.get("reviewed_at") or ""
        if not ts_raw:
            continue
        try:
            # 兼容 'Z' 后缀（fromisoformat 3.10 不支持 'Z'，3.11 起原生支持）
            normalized = ts_raw.replace("Z", "+00:00") if ts_raw.endswith("Z") else ts_raw
            ts = datetime.fromisoformat(normalized)
            # 给 naive datetime 补 UTC（理论上 _now_iso 写入都是带时区，但保守）
            if ts.tzinfo is None:
                ts = ts.replace(tzinfo=timezone.utc)
        except (ValueError, TypeError):
            # 解析失败 → 保守归为 stale（让人看见）
            stale.append(d)
            continue
        if ts < threshold:
            stale.append(d)
