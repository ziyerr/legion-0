...
6 步推理链（详见 ARCHITECTURE.md §1 / RECON-HISTORY 8.4）：
  ...
  2. 检查 ADR 历史（保持决策连贯性）
  3. 查 EngineerProfile（Phase 1 hardcoded 8 军团）
  4. LLM 推理生成 6 字段 JSON（含 red verdict 改进路径强约束）
  5. 每个 tech_stack 选项写一条 ADR
  6. 渲染飞书技术方案文档（不阻塞主流程）
...
from . import adr_storage, error_classifier, feishu_api, pm_db_api
...
def run(args: Dict[str, Any], **kwargs) -> str:
    ...
    # ---- Step 3：EngineerProfile（Phase 1 hardcoded） ----
    legion_info = HARDCODED_LEGION_PROFILES

    # ---- Step 4：LLM 推理生成 6 字段 JSON ----
    try:
    ...
    # ---- Step 5：每个 tech_stack 选项写一条 ADR ----
    adr_ids: List[str] = []
