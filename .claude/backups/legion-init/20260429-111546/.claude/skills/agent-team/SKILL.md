...
> 专用 agent 已内置初始化协议（读需求/读 CLAUDE.md/读审查清单等），prompt 只需提供任务上下文。
...
```bash
# 空降审查（伞兵提供第二意见）
bash .claude/scripts/codex-team.sh review --base main gui/src/components/Foo.tsx

# 空降红队（差异模型视角的对抗性审查）
bash .claude/scripts/codex-team.sh adversarial --base main

# 空降救援（团队卡壳时，伞兵独立调查）
bash .claude/scripts/codex-team.sh rescue "video_gen 在并发 >5 时死锁，已排查 timeout 和 mutex"

# 空降参谋（对技术方案征求不同视角）
bash .claude/scripts/codex-team.sh second-opinion "DAG 调度器用 petgraph 还是自研拓扑排序？"
```

**核心原则：差异模型的价值在于不同视角。** Codex（GPT）和 Claude 的认知框架不同，同一段代码可能关注不同的风险点。正因为它是"劲敌"，它的不同意见最有价值。

**使用场景（M 级及以上全部强制）：**
- M 级验证阶段 → 伞兵 review 与 Claude verify 交叉验证
...
所有审查完成后，进入对抗性验证（使用 `subagent_type: "verify"`，读 .claude/skills/audit/SKILL.md）：
...
```bash
# 检查文件是否被 claim
bash .claude/scripts/claims.sh check gui/src/App.tsx

# claim 文件（冲突时自动报错）
bash .claude/scripts/claims.sh claim gui/src/App.tsx L1-青龙军团 "添加导航入口"

# 列出所有 claim
bash .claude/scripts/claims.sh list

# 释放（任务完成后）
bash .claude/scripts/claims.sh release L1-青龙军团
```
...
1. **释放 claim** — `bash .claude/scripts/claims.sh release L1-xxx`
2. **更新简报** — `.claude/commander/briefings/{自己的L1名}.md`：
   - 旧经验过时 → 直接修改（自己的或别人的）
   - 新增：复杂度校准、编排实效、高风险模块
   - 保持精简（≤50行），过时的删除
