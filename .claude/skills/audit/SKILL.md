...
| **L 级** | 合规 + 红队 + **伞兵红队** | 2+1 | **强制** | Claude 红队 + 伞兵对抗性审查 |
| **XL 级** | 合规 + 红队 + 集成 + **伞兵红队** | 3+1 | **强制** | 最大规模验证，伞兵作为独立红队成员 |

### 伞兵红队成员

伞兵（Codex/GPT）作为军团正式外援参与验证。**差异模型的价值在于不同的认知框架** — 同一段代码，Claude 和 GPT 关注不同的风险点，交叉验证覆盖更全面。

L/XL 级验证时，在部署 Claude verify agent 的同时，空降伞兵对抗性审查：

```bash
# 伞兵空降，与 Claude verify agent 并行执行
bash .claude/scripts/codex-team.sh adversarial --base main
```

**伞兵红队报告处理规则：**
- 伞兵发现的 critical/major → 必须在 Claude 验证者报告中交叉确认或明确反驳
- 伞兵与 Claude 验证者意见不同 → 指挥官仲裁，不可忽略任一方
- 伞兵发现 Claude 全部验证者都没注意到的问题 → 高价值发现，记入战法库
...
指挥官直接运行：
```bash
bash ~/.claude/scripts/stack-verify.sh full
```
贴输出，确认通过即可。
...
1. **意外踩坑？** → 有 → 写战法到 `~/.claude/memory/tactics/`
2. **非常规有效路径？** → 有 → 写战法
