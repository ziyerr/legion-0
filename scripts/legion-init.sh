...
validate_skills_directory() {
  ...
  if [[ "$invalid_count" -gt 0 ]]; then
    echo -e "  ${YELLOW}⚠ skills 目录已清理 ${invalid_count} 个非目录/链接条目${NC}"
  else
    echo "  ✓ skills 目录结构已检查"
  fi
}
...
merge_claude_execution_discipline() {
  ...
  merged_file=$(mktemp "${claude_file}.merged.XXXXXX")
  CLAUDE_FILE="$claude_file" CLAUDE_MERGED_FILE="$merged_file" python3 << 'PY'
from pathlib import Path
import os
import re

path = Path(os.environ["CLAUDE_FILE"])
out_path = Path(os.environ["CLAUDE_MERGED_FILE"])
...
def compact_legacy(value: str, budget: int) -> str:
    value = normalize(value)
    ...
    notice = "（历史 CLAUDE.md 已自动压缩；完整原文见 .claude/backups/legion-init/）"
    sections = []
    current = []
...
legacy = strip_marker_blocks(text)
legacy = strip_history_wrapper(legacy)
legacy = strip_legacy_discipline_sections(legacy)
legacy = normalize(legacy)

sep = "\n\n---\n\n"
if legacy:
    budget = MAX_CHARS - len(block) - len(sep) - len("# 历史 CLAUDE.md（已压缩）\n\n") - 1
    legacy = compact_legacy(legacy, budget)
    merged = block + sep + "# 历史 CLAUDE.md（已压缩）\n\n" + legacy
else:
    merged = block

if len(merged) > MAX_CHARS:
    prefix = block + sep + "# 历史 CLAUDE.md（已压缩）\n\n"
    legacy_budget = MAX_CHARS - len(prefix) - 1
    merged = prefix + compact_legacy(legacy, legacy_budget)

out_path.write_text(merged.rstrip() + "\n", encoding="utf-8")
PY
  if cmp -s "$claude_file" "$merged_file"; then
    rm -f "$merged_file"
    echo "  → CLAUDE.md 已是最新压缩纪律模板，跳过"
  else
    backup_existing_file "$claude_file"
    mv "$merged_file" "$claude_file"
    echo -e "  ${GREEN}✓ CLAUDE.md 已备份并合并压缩最新纪律模板${NC}"
  fi
}
...
validate_skills_directory "${TARGET_DIR}/.claude/skills"
...
echo "  3. claudel1        # 启动/恢复 Claude L1 军团"
echo "     codexl1         # 启动/恢复 Codex L1 军团"
echo ""
echo "  快速验证："
