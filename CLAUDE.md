...
3. **最大有效规模，不是盲目重复** — 扩编必须带来互补视角或并行吞吐，禁止多个军团无差别重复同一动作。
4. **质量门前置且独立** — 实现与审计/验证必须分离；中型及以上任务默认引入独立 review / verify / audit 分支。
5. **上下文即资产** — L2 是否解散取决于其持有上下文是否仍有价值；需要后续迭代、失败诊断、复杂背景延续时保留。

本原则中的“不计成本”指资源消耗成本，不覆盖下面四种例外。
